...
<x-forms.patch :action="route('frontend.user.profile.update')">
    ...
    <h2>@lang('frontend_users.account.profile')</h2>
    <div class="form-group row ">
        <label for="name" class="col-md-3 col-form-label-sm text-md-right">@lang('general.user_name')</label>

        <div class="col-md-9">
            <input type="text" name="name" class="form-control form-control-sm" placeholder="{{ __('general.user_name') }}" value="{{ old('name') ?? $logged_in_user->name }}" required autofocus autocomplete="name" />
        </div>
    </div><!--form-group-->
...
{{--    </div><!--form-group-->--}}
    ...
    <div class="form-group row">
        <label for="department_id" class="col-md-3 col-form-label-sm text-md-right">@lang('general.department')</label>
    ...
    @if ($logged_in_user->canChangeEmail())
        <div class="form-group row">
            <label for="email" class="col-md-3 col-form-label-sm text-md-right">@lang('general.email')</label>
            ...
            <div class="col-md-9">
                ...
                <input type="email" name="email" id="email" class="form-control form-control-sm" placeholder="{{ __('general.email') }}" value="{{ old('email') ?? $logged_in_user->email }}" required autocomplete="email" />
            </div>
        </div><!--form-group-->
    @endif
    ...
    <div class="form-group row mb-0">
        <div class="col-md-12 text-right">
            <button class="btn btn-sm btn-primary float-right" type="submit">@lang('frontend_users.account.update')</button>
        </div>
    </div><!--form-group-->
</x-forms.patch>
