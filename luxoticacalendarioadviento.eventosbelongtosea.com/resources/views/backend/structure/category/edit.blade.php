...
@section('title', __('Actualizar Categoría'))
...
@section('content')
    <x-forms.patch :action="route('admin.structure.category.update', $category)" enctype="multipart/form-data">
        <x-backend.card noPadding>
            <x-slot name="header">
                @lang('Actualizar Categoría')
            </x-slot>
                ...
                <div class="card">
                    ...
                    <div class="row px-4">
                        <div class="form-group col-md-4">
                            ...
                            <input type="number" name="order" class="form-control" placeholder="{{ __('Orden') }}" value="{{ old('order') ?? $category->order}}" maxlength="3" required />
                        </div><!--form-group-->

                        <div class="form-group col-md-4">
                            <label for="published">@lang('Publicada')</label>
                            <select name="published" class="form-control" required>
                                <option value="0" {{ $category->published == false ? 'selected' : '' }}>@lang('No')</option>
                                <option value="1" {{ $category->published == true ? 'selected' : '' }}>@lang('Sí')</option>
                            </select>
                        </div><!--form-group-->
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select name="active" class="form-control" required>
                                <option value="0" {{ $category->active == false ? 'selected' : '' }}>@lang('No')</option>
                                <option value="1" {{ $category->active == true ? 'selected' : '' }}>@lang('Sí')</option>
                            </select>
                        </div><!--form-group-->
                    </div>
                </div>
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <x-forms.img-selector classBrowseBtn="btn btn-sm btn-primary mr-3 align-top"
                                                       ...
                                                       minResFactor="4" minResFactor="3" maxResFactor="50" src="{{ old('icon') ?? $category->iconURL }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <textarea class="form-control" name="description" rows="3" placeholder="Descripción">{{ old('description') ?? $category->description }}</textarea>
                            </div>
                        </div><!--form-group-->
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <x-forms.img-selector classBrowseBtn="btn btn-sm btn-primary mr-3 align-top"
                                                       classDeleteBtn="btn btn-sm btn-danger mr-3 align-top"
                                                       classPreview="border-dark"
                                                       id="2" name="image" width="320" height="98" resFactor="8" minResFactor="5" maxResFactor="50" src="{{ old('image') ?? $category->imageURL }}">
                                </x-forms.img-selector>
                            </div>
                        </div><!--form-group-->
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <textarea class="form-control tinymce" name="content" rows="3" placeholder="Contenido">{{ old('description') ?? $category->content }}</textarea>
                            </div>
                        </div><!--form-group-->
                    </div>
                </div>
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Actualizar Categoría')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.patch>
@endsection
