                ...
                <x-forms.post id="response-comment-modal-form-{{ $response->id }}" action="{{ route('frontend.responses.comment', ['response' => $response->id]) }}">
                    <div class="form-group row">
                        <div class="col-md-12 py-1">
                            <textarea id="response-comment-modal-form-input-{{ $response->id }}" class="form-control emoji-text-input"  name="content" rows="3" placeholder="@lang('components_frontend.response.comment.comment_placeholder')">   </textarea>
                        </div>
                    </div><!--form-group-->
                </x-forms.post>
