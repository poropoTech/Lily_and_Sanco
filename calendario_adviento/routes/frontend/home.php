...
use App\Http\Controllers\Frontend\PrizesController;
...
Route::group(['middleware' => ['auth', 'password.expires', config('boilerplate.access.middleware.verified')]], function () {
    ...
    Route::get('c', [CategoryController::class, 'index'])
        ...
        ->breadcrumbs(function (Trail $trail) {
            $trail->parent('frontend.index')
                ->push(__('Propósitos'), route('frontend.pages.categories'));
        });
    ...
    Route::get('premios', [PrizesController::class, 'index'])
        ->name('pages.prizes')
        ->breadcrumbs(function (Trail $trail) {
            $trail->parent('frontend.index')
                ->push(__('Premios'), route('frontend.pages.prizes'));
        });
});
