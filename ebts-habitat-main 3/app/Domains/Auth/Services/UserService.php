...
class UserService extends BaseService
{
    ...
    public function registerUser(array $data = []): User
    {
        ...
        try {
            ...
            if (isset($data['avatar'])) {
                $user->addMediaFromBase64($data['avatar'])
                    ->usingName('Avatar')
    ...
    {
        if ($data['type'] == $this->model::TYPE_ADMIN) {
            $dept_id = null;
        } else {
    ...
    public function update(User $user, array $data = []): User
    {
        ...
        if ($user->type == $this->model::TYPE_ADMIN) {
            $dept_id = null;
        } else {
            $dept_id = $data['department_id'];
        }
    ...
    public function updateProfile(User $user, array $data = []): User
    {
        ...
        $user->alias = $data['name'] ?? null;
        $user->department_id = $data['department_id'] ?? null;

        if (isset($data['avatar'])) {
            $user->deleteAvatar();
            $user->addMediaFromBase64($data['avatar'])
                ->usingName('Imagen de perfil')
                ->usingFileName(getRandomFilenameFromB64($data['avatar']))
                ->toMediaCollection('avatar');
        }
